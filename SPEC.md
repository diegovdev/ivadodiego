...
| B21 | . | 2026-05-07 | T10 | low | `load_model()` not in T10 spec (T10 = `train` + `predict` + `InsufficientDataError`) | premature implementation of T11 "load model at startup" | keep — T11 will need it; record as premature, not harmful |
| B22 | . | 2026-05-07 | T10 | medium | `predict()` raises bare `RuntimeError` when model absent — §I says `POST /predict` → 400 if model absent; API layer must catch specific exception | generic `RuntimeError` is fragile boundary; any other RuntimeError also triggers 400 | add `ModelNotLoadedError(RuntimeError)` alongside `InsufficientDataError`; T11 API layer catches by type |
