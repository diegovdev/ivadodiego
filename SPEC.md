...
| T2 | x | scaffold | Bootstrap: `pyproject.toml` (uv, deps, dev-group), `src/museums/__init__.py`, module stubs (`scraper.py`, `enricher.py`, `db.py`, `regression.py`, `api.py`), `tests/conftest.py`, `notebooks/`, `models/`, ruff+ANN config, logging config | C | #2 | feature/scaffold |
| T3 | x | scaffold | Docker: multi-stage `Dockerfile` (builder + `python:3.12-alpine`, uid 1000, apk build deps for compiled packages), `docker-compose.yml` (`api`:8000 + `jupyter`:8888, vol `museums-data`, healthcheck, depends_on) | V13,V14,V21,V23,V37,I.compose | #3 | feature/scaffold |
| T4 | x | scaffold | API skeleton `src/museums/api.py`: FastAPI app, `GET /health`, `GET /docs`, route stubs returning empty Pydantic payloads — boots in container | V11,V12,I.api | #4 | feature/scaffold |
