...
| B36 | . | 2026-05-08 | T17,T22 | medium | `ecs.py:115` `awslogs-region` hardcoded `us-east-1`; `networking.py:77` AZ names hardcoded `us-east-1{az}` | region from config not threaded to ecs/networking modules | pass region parameter through `create()` signatures; derive AZs from region |
| B37 | . | 2026-05-08 | T24 | high | `nightly.yml:65` `pip-audit --requirement <(uv export ...)` uses bash process substitution but Alpine container shell = sh (dash) | bashism in sh context → silent failure or parse error | pipe: `uv export --no-dev --format requirements-txt \| pip-audit -r /dev/stdin` |
| B38 | . | 2026-05-08 | T24 | medium | `nightly.yml:41-43` hard `time.sleep(30)` for ingest wait — if ingest takes >30s, verify step sees 0 rows → false failure | no polling, just fixed sleep | poll `/status` endpoint until `ingest.status != "never"` with timeout |
| B39 | . | 2026-05-08 | T23 | medium | `cd.yml:24,66` shared `AWS_DEPLOY_ROLE_ARN` for staging+prod — single IAM role across envs | lateral movement: staging job can mutate prod | split to `AWS_STAGING_DEPLOY_ROLE_ARN` / `AWS_PROD_DEPLOY_ROLE_ARN` |
| B40 | . | 2026-05-08 | T23 | medium | `cd.yml` deploy-staging has no `needs:` on any validation job; deploy-prod has no `needs:` on deploy-staging | broken image/config reaches staging unchecked; prod can deploy without staging verification | add preflight validation step; chain deploy-prod after deploy-staging |
| B41 | . | 2026-05-08 | T25 | low | `nightly.yml:94` `pip install locust --quiet` unpinned — V42 precedent says pin tool versions | floating locust version can break nightly | pin to specific version or add to dev deps |
