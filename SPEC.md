...
| T26 | x | delivery | `.github/workflows/release.yml` — triggers on push to `main`; Alpine container; full history checkout; `semantic-release version` (bumps pyproject.toml + tag) + `semantic-release publish` (GitHub Release + CHANGELOG); uses `GITHUB_TOKEN` | V25,V37,V38,T6 | - | - |
...
| B41 | x | 2026-05-08 | T25 | low | `nightly.yml:94` `pip install locust --quiet` unpinned — V42 precedent says pin tool versions | floating locust version can break nightly | pin to specific version or add to dev deps |
